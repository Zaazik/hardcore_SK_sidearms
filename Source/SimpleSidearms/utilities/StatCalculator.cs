...
namespace SimpleSidearms.utilities
{
    public static class StatCalculator
    {
        ...
        internal static float GetUnarmdeMeleeDPS(Pawn pawn)
        {
            if (pawn == null)
            {
                Log.Error("pawn can't be nil");
                return 0f;
            }
            
            return GetAdjustedMeleeDamageUnarmedAmount(pawn) * GetMeleeHitChance(pawn) / GetMeleeUnarmedCooldown(pawn);
        }

        internal static float GetMeleeUnarmedCooldown(Pawn pawn)
        {
            List<VerbEntry> availableVerbsList = pawn.meleeVerbs.GetUpdatedAvailableVerbsList(false);
            if (availableVerbsList.Count == 0)
                return 1f;
            
            float num1 = 0.0f;
            for (int index = 0; index < availableVerbsList.Count; ++index)
            {
                if (availableVerbsList[index].IsMeleeAttack)
                {
                    if (availableVerbsList[index].verb.ImplementOwnerType.implementOwnerRuleName != "weapon")
                    {
                        if (availableVerbsList[index].verb.EquipmentSource == null)
                        {
                            num1 += availableVerbsList[index].GetSelectionWeight((Thing) null);
                        }
                    }
                }
            ...
            for (int index = 0; index < availableVerbsList.Count; ++index)
            {
                if (availableVerbsList[index].IsMeleeAttack)
                {
                    if (availableVerbsList[index].verb.ImplementOwnerType.implementOwnerRuleName != "weapon")
                    {
                        if (availableVerbsList[index].verb.EquipmentSource == null)
                        {
                            num2 += availableVerbsList[index].GetSelectionWeight((Thing) null) / num1 *
                                    (float) availableVerbsList[index].verb.verbProps
                                        .AdjustedCooldownTicks(availableVerbsList[index].verb, pawn);
                        }
                    }
                }
            }
        ...
        internal static float GetAdjustedMeleeDamageUnarmedAmount(Pawn pawn)
        {
            List<VerbEntry> availableVerbsList = pawn.meleeVerbs.GetUpdatedAvailableVerbsList(false);
            if (availableVerbsList.Count == 0)
            {
                Log.Error("something wong availableVerbsList.Count==0");
                return 0;
            }
            ...
            if (num1 == 0.0)
            {
                Log.Error("something wong num1 == 0.0");
                return 0;
            }
            ...
            for (int index = 0; index < availableVerbsList.Count; ++index)
            {
                if (availableVerbsList[index].IsMeleeAttack)
                {
                    if (availableVerbsList[index].verb.ImplementOwnerType.implementOwnerRuleName!="weapon")
                    {
                        ...
                        {
                            num2 += availableVerbsList[index].GetSelectionWeight((Thing) null) / num1 *
                                    availableVerbsList[index].verb.verbProps
                                        .AdjustedMeleeDamageAmount(availableVerbsList[index].verb, pawn);
                        }
                    }
                }
            }
